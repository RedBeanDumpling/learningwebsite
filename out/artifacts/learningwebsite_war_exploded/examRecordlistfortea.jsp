...
<!-- AddThis Smart Layers END -->
	...
	<div class="contact-form">
					...
					<form action="examRecordListForTeacher.do" method="post" name="regForm">
						      ...
						      <div class="layui-input-inline">
						        <select name="pid" id="pid" lay-search="">
									  	...
									  	<c:forEach items="${plist}" var="p">
									  	  <option value="${p.id}" ${pid eq p.id?"selected":"" }>《${p.kechen.kcname}》${p.title}</option>
									  	</c:forEach>
								  	</select>
						      </div>
						    </div>
					...
					<tbody class="news_content">
					   <c:forEach items="${pageInfo.list}" var="exam">
						<tr>
							<th>${exam.paper.kechen.kcname}</th>
							<td style="text-align: left;width: 400px">${exam.paper.title}</td>
							<td>${exam.score}分</td>
							<td>${exam.student.tname}</td>
							<td>
							<a class="layui-btn  layui-btn-mini" href="examResult.do?id=${exam.id}">查看试卷</a>
							<a class="layui-btn  layui-btn-mini" href="examDelforTeacher.do?id=${exam.id}">删除</a>
							</td>
						</tr>
						</c:forEach>
						<tr>
        ...
        layui.use(['laypage', 'layer'], function(){
            ...
            laypage.render({
                ...
                ,jump: function(obj,first){
                    ...
                    if (!first) {
                        location.replace("examRecordListForTeacher.do?pageNum="+currentpage+"&pageSize="+pageSize+"&pid="+pid)
                    }
