...
<!-- AddThis Smart Layers END -->
	...
	<div class="contact-form">
				    ...
				    <button style="background:#003B64;color:white;width: 80px;height: 35px;border:0px; " onclick="add('skipkechenlr.do?kcid=${kcid}&flag=课程视频&biaoji=kechenlradd')">上传视频</button>
					...
					<tbody class="news_content">
					   <c:forEach items="${pageInfo.list}" var="kcvideo">
						<tr>
							<td>${kcvideo.title}</td>
							<td>${kcvideo.savetime}</td>
                            <td>${kcvideo.cs}</td>
							<td>${kcvideo.shstatus}</td>
							<td>
							   <a class="layui-btn layui-btn-mini" href="skipkechenlr.do?id=${kcvideo.id}&kcid=${kcid}&flag=课程视频&biaoji=kechenlredit">编辑</a>
							   <a class="layui-btn layui-btn-danger layui-btn-mini" href="kcvedeoSc.do?id=${kcvideo.id}">删除</a>
							</td>
						</tr>
						</c:forEach>
						<tr>
        ...
        layui.use(['laypage', 'layer'], function(){
            ...
            laypage.render({
                ...
                ,jump: function(obj,first){
                    ...
                    if (!first) {
                        location.replace("kcvideoLb.do?pageNum="+currentpage+"&pageSize="+pageSize+"&kcid="+kcid)
                    }
